...
def calc_metrics(pred_data, gt_data):
    ...
    accuracy = (tp + tn) / total_voxeles
    specificity = tn / (tn + fp + 1e-9)
    dice = (2. * tp) / (2 * tp + fp + fn + 1e-9)
    recall = tp / (tp + fn + 1e-9)
    precision = tp / (tp + fp + 1e-9)
    return dice, pixel_wise_accuracy, pixel_wise_precision, ious_por_clase, specificity, recall, precision, accuracy, tp, tn, fp, fn
...
def save_results(output_folder, resultados):
    ...
    # Clases del IoU
    clases = sorted(set(clase for _, _, _, _, ious_por_clase, _, _, _, _, _, _, _, _ in resultados for clase in ious_por_clase))
    
    # Create CSV
    with open(output_path, mode='w', newline='') as csv_file:
        writer = csv.writer(csv_file)
        # Encabezado
        header = ["File", "Dice", "Pixel-Wise Accuracy", "Pixel-Wise Precision", "Specificity", "Recall", "Precision" , "Accuracy", "TP", "TN", "FP", "FN"] + [f"IoU_clase_{clase}" for clase in clases]
        writer.writerow(header)
        
        # Datos
        for pred_file, dice, pixel_wise_accuracy, pixel_wise_precision, ious_por_clase, specificity, recall, precision, accuracy, tp, tn, fp, fn in resultados:
            # Crear la fila con los valores básicos y las métricas por clase
            row = [pred_file, dice, pixel_wise_accuracy, pixel_wise_precision, specificity, recall, precision, accuracy, tp, tn, fp, fn] + [ious_por_clase.get(clase, "") for clase in clases]
            writer.writerow(row)
...
def process_folders(pred_folder, gt_folder, output_folder):
    ...
    for pred_file, gt_file in zip(archivos_pred, archivos_gt):
        ...
        if verify_dimensions(pred_img, gt_img) and verify_affinity(pred_img, gt_img):
            ...
            dice, pixel_wise_accuracy, pixel_wise_precision, ious_por_clase, specificity, recall, precision, accuracy, tp, tn, fp, fn = calc_metrics(pred_data, gt_data)
            results.append((pred_file, dice, pixel_wise_accuracy, pixel_wise_precision, ious_por_clase, specificity, recall, precision, accuracy, tp, tn, fp, fn))
