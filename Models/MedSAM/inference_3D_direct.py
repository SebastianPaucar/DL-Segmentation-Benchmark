...
def MedSAM_infer_npz(gt_path_file):
    ...
    if (not isfile(join(pred_save_dir, task_folder, npz_name))) or overwrite:
        ...
        img_roi_sitk = sitk.GetImageFromArray(seg_3D)
        #img_roi_sitk = sitk.GetImageFromArray(gt_3D)
        img_roi_sitk.SetSpacing(spacing)
        sitk.WriteImage(
            img_roi_sitk,
                join(pred_save_dir,task_folder, npz_name.split(".")[0] + "_seg.nii.gz"),
        )
